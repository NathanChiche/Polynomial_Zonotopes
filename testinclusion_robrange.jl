...
function paverobust(f,dim,nbvars,epsilon,quantifiers)
    ...
    while width>epsilon
        ...
        tour=(tour)%dim+1
        width=last(last(listintervalbox)).hi-last(last(listintervalbox)).lo
    end
    return 0
end 
...
paverobust([h_1,h_2],2,3,1.5,["forall","forall","exists"])
...
inp=[interval(-1..1) for i in 1:4]
bisect_at_component(inp,2)
