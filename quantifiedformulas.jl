...
comb=collect(combinations([0,1,2,3,4],3))
binomial(5,3)
comb
comb2=collect(Iterators.product(0:1,0:1,0:1,0:1))

function range_find(g,nb_vars)
...
function make_quant() # CHANGEMENT NECESSAIRE CEST POURRI
    q1=["forall","forall","exists","forall"]
    q2=["forall","forall","forall","exists"]
    return [[q1,q2],[q2,q1]]
end
...
#=I([interval(-1.0,2.0),interval(0,1.0)],1)
inter=interval(-1.0,2.0)
abs(inter)=#
...
function call_multiple_outers(nD_funct,centers,quantifiers,input,x)
    ...
    for j in 1:length(nD_funct)
        ...
        #println(range_Dg)
        i=outer_approx(range_Dg,centers[j],quantifiers)
        if !(0 in i)
...
weval(sinx; x=2.0)
#a=W"Reduce" W"[" W"Exists" W"[" W"{x, y}" W"," W"x^2 + a*y^2 <= 1 && x - y >= 2" W"]" W"," W"a" W"," W"Reals" ]
weval(a)